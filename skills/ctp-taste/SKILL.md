---
name: ctp-taste
description: Anti-slop frontend skill for landing pages, portfolios, and redesigns. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check. Dans le parcours Collectif Tariqa PRO, c'est le moteur de design appelé par `ctp-page-de-vente` pour produire une landing page qui ne fait pas "AI slop".
metadata:
  version: 1.3.0
  category: tariqa-pro
---
